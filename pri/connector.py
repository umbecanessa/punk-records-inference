...
def _audit_resume_config_or_abort(cfg: dict) -> dict | None:
    ...
    try:
        ...
        audit = summarize_geometry_audit(
            cfg.get("snapshots", []),
            rope_offset=0,
            resume_mode=True,
...
class NLSSnapshotConnector(KVConnectorBase_V1, SupportsHMA):
    ...
    def __init__(
        ...
        self._rope_head_dim = getattr(hf, "head_dim", 256)
        self._rope_theta = getattr(hf, "rope_theta", 10_000_000)
        self._num_kv_heads = getattr(hf, "num_key_value_heads", 2)
    ...
    def _inject_kv(
        ...
        if len(shape) == 5 and shape[0] == 2:
            ...
            dst_kv_cache[block_idxs, 0, offsets] = k.reshape(num_tokens, -1)
            dst_kv_cache[block_idxs, 1, offsets] = v.reshape(num_tokens, -1)
