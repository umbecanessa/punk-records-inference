...
async def _system_and_tools_token_count(
    ...
    *,
    ...
    composite_key = _sys_prompt_hash(system_content + "\u0000" + tools_json)
    cached = _sys_prompt_token_cache.get(composite_key)
    if cached is not None:
        return cached
    ...
    try:
        ...
        user_only_body: dict[str, Any] = {
            "messages": [{"role": "user", "content": dummy_user}],
            "chat_template_kwargs": {"enable_thinking": False},
            "add_generation_prompt": False,
        }
        if model:
            with_tools_body["model"] = model
            user_only_body["model"] = model
        with_data, user_data = await asyncio.gather(
            _post_json(f"{base}/tokenize", with_tools_body),
            _post_json(f"{base}/tokenize", user_only_body),
        )

        def _count(data: dict[str, Any]) -> int:
            c = data.get("count")
            if c is None and isinstance(data.get("tokens"), list):
                c = len(data["tokens"])
            return int(c or 0)

        count = max(0, _count(with_data) - _count(user_data))
        _sys_prompt_token_cache[composite_key] = count
        return count
    except Exception as exc:
        logger.warning("tokenize(messages+tools) fallback: %s", exc)
        return await _token_count_for(
            request, _render_system_turn_like_qwen(system_content), model=model
        )
...
async def _apply_agent_shim(request: Request, body: dict[str, Any]) -> dict[str, Any]:
    ...
    if system_content:
        if body.get("tools"):
            ...
            capture_start = await _token_count_for(
                request, _render_system_turn_like_qwen(system_content), model=model
            )
