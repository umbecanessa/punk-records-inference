...
**Long-term vs session memory:** Full NLS includes DomainDB + LoRA weight chain over days ([JOURNEY](JOURNEY.md)). Chain resume = transformer state within a thread ([NLS pipeline § inject modes](NLS_PIPELINE.md#injection-modes-retrieval-vs-chain-resume)).

---

| Not PRI | Notes |
|---------|-------|
| Text/context compression | Shrinks the wire — GPU may still re-run attention; PRI skips re-prefill via KV inject |
