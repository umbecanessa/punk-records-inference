...
- RoPE re-rotation assumes Qwen hybrid head layout on pinned vLLM (Tier A)
- **Tier B** (Gemma, Llama): K/V inject only — no Mamba delta-sum; recall curves differ from Qwen ([`MODEL_MATRIX.md`](MODEL_MATRIX.md))
