...
pip install pytest torch zstandard requests
pytest tests/ -q
```
...
User-facing docs live under `docs/` with entry at [docs/index.md](docs/index.md). Maintainer-only planning notes stay local (`docs/internal/`, gitignored) — do not commit them.
...
Open a [GitHub issue](https://github.com/umbecanessa/punk-records-inference/issues) for questions, bugs, or reproduction reports.
