...
DATA_VOLUME="${PRI_DATA_VOLUME:-pri-data}"
...
if docker inspect "$CONTAINER" >/dev/null 2>&1; then
  run_step "copy_model_profile" bash -c "
    docker cp ${CONTAINER}:/data/pri/model_profile.json '${OUT_DIR}/model_profile.json' 2>/dev/null || \
    docker cp ${CONTAINER}:${NLS_MEMORY_DIR:-/data/pri}/model_profile.json '${OUT_DIR}/model_profile.json'
  " || true
  docker cp "${CONTAINER}:/data/pri/profile.env" "${OUT_DIR}/profile.env" 2>/dev/null || true
fi
...
if [[ "${SKIP_WIPE:-0}" != "1" ]]; then
  run_step "wipe_memory" env PRI_CONTAINER="$CONTAINER" PRI_DATA_VOLUME="$DATA_VOLUME" \
    "${ROOT}/bench/wipe_memory.sh" --restart || true
...
run_step "turn_sweep" python3 -u "${ROOT}/bench/tier1/turn_sweep.py" \
  ...
  --garbled-retries 2 \
  $(if [[ "${SWEEP_NO_STOP_GARBLE:-0}" == "1" ]]; then echo "--no-stop-on-noise-garble"; else echo "--stop-on-noise-garble"; fi) \
  --out "$SWEEP_JSON"
